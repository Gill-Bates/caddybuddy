...
| `otp_login.html` | Second-factor prompt after a correct password (`app/routers/ui/auth.py`). |
| `two_factor_setup.html` | TOTP enrollment page with QR code, secret, and confirmation form (`app/routers/ui/settings.py`). |
| `two_factor_recovery.html` | One-time display of recovery codes after enrollment (`app/routers/ui/settings.py`). |
