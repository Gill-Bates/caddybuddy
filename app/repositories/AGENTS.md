...
| File | Description |
|------|-------------|
| `users.py` | `UserRepository` + `DuplicateUserError`; username/email normalization and password-hash/role validation helpers; OTP state changes are conditional atomic `UPDATE`s (enrollment only while disabled, strictly increasing TOTP counter, compare-and-swap on recovery-code hashes). |
